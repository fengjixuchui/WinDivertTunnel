...
void server_ipv4::send_response_packet(PVOID packet, UINT recv_len)
{
...
void server_ipv4::set_packet_template(PVOID packet, UINT recv_len)
{
	PWINDIVERT_TCPHDR tcp_header;
	PWINDIVERT_IPHDR ip_header;
	UINT payload_len;
...
void server_ipv4::send_data_packet(const char* payload_buf, int payload_len)
{
	PWINDIVERT_TCPHDR tcp_header;
	PWINDIVERT_IPHDR ip_header;
...
void server_ipv4::send_connect_reponse(PVOID packet, UINT recv_len)
{
	PWINDIVERT_TCPHDR tcp_header;
	PWINDIVERT_IPHDR ip_header;
	WinDivertHelperParsePacket(packet, recv_len, &ip_header, NULL,
...
void server_ipv4::print_ip_info(PVOID ip_header)
{
	char dst_addr[16];
	char src_addr[16];
...
void server_ipv4::wait_for_connect()
{
	UINT recv_len;
...
bool server_ipv4::connect_to_target()
{
...
void server_ipv4::build_packet_template()
{
...
void server_ipv4::build_addr_template()
{
...
{
	return 1 != inet_pton(AF_INET, laddr.c_str(), m_laddr);
}

bool server_ipv4::set_raddr(std::string raddr)
{
	return 1 != inet_pton(AF_INET, raddr.c_str(), m_raddr);
}