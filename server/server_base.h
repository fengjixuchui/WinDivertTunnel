...
private:
	...
	virtual bool connect_to_target() = 0;
	virtual void build_packet_template() = 0;
	virtual void build_addr_template() = 0;
	virtual void print_ip_info(PVOID ip_header) = 0;
	virtual bool recv_data_packet(PVOID packet_buf, PUINT payload_len, PVOID* payload_buf) = 0;
	virtual void wait_for_connect() = 0;
	virtual void send_connect_reponse(PVOID packet, UINT recv_len) = 0;								// 模拟tcp三次握手
	virtual void set_packet_template(PVOID packet, UINT recv_len) = 0;								// 格式化标准发送包
	virtual void send_response_packet(PVOID packet, UINT recv_len) = 0;								// 发送确收包
	virtual void send_data_packet(const char* payload_buf, int payload_len = 0) = 0;				// 发送数据包
